...
//lwip DHCP任务
//设置任务优先级
#define LWIP_DHCP_TASK_PRIO       		(tskIDLE_PRIORITY + 5) 
//设置任务堆栈大小
#define LWIP_DHCP_STK_SIZE  		    128
//任务堆栈，采用内存管理的方式控制申请	
StackType_t *LWIP_DHCP_TASK_STK;	
//任务控制块
StaticTask_t LWIP_DHCP_TASK_BUFF;
//任务名字
const char *LWIP_DHCP_TASK_NAME = "task DHCP";
//任务函数
void lwip_dhcp_task(void *pdata); 

//lwip DM9000数据接收处理任务
//设置任务优先级
#define LWIP_DM9000_INPUT_TASK_PRIO		(tskIDLE_PRIORITY + 7) 
//设置任务堆栈大小
#define LWIP_DM9000_INPUT_TASK_SIZE	    512
//任务堆栈，采用内存管理的方式控制申请	
StackType_t  *LWIP_DM9000_INPUT_TASK_STK;
//任务控制块
StaticTask_t LWIP_DM9000_INPUT_TASK_BUFF;
//任务名字
const char *LWIP_DM9000_INPUT_TASK_NAME = "task DM9000 input";
//任务函数
void lwip_dm9000_input_task(void *pdata); 


StackType_t *TCPIP_THREAD_TASK_STK;			//lwip内核任务堆栈  
StaticTask_t LWIP_THREAD_TASK_BUFF;			//lwip内核任务控制块

SemaphoreHandle_t dm9000input;					//DM9000接收数据信号量
SemaphoreHandle_t dm9000lock;					//DM9000读写互锁控制信号量
...
//	OS_CPU_SR cpu_sr;
	...
	if(lwip_comm_mem_malloc())return 1;	//内存申请失败
 	...
 	dm9000input=xSemaphoreCreateCounting(10, 0);			//创建数据接收信号量,必须在DM9000初始化之前创建
	dm9000lock=xSemaphoreCreateMutex();	//创建互斥信号量,提高到优先级2	
...
//	taskENTER_CRITICAL();  		//进入临界区
	xTaskCreateStatic(lwip_dm9000_input_task,
					  LWIP_DM9000_INPUT_TASK_NAME,
					  (uint32_t)512,
					 (void*)NULL,
					  (UBaseType_t)LWIP_DM9000_INPUT_TASK_PRIO,
					 (StackType_t *const)LWIP_DM9000_INPUT_TASK_STK,
					 (StaticTask_t *const)&LWIP_DM9000_INPUT_TASK_BUFF);		 //以太网数据接收任务
