...
extern SemaphoreHandle_t dm9000input;				//DM9000接收数据信号量
extern SemaphoreHandle_t dm9000lock;				//DM9000读写互锁控制信号量
//初始化DM9000
//返回值:
//0,初始化成功
//1，DM9000A ID读取错误
...
void DM9000_SendPacket(struct pbuf *p)
{
	...
	u8 err;	
 	//printf("send len:%d\r\n",p->tot_len);
	err = xSemaphoreTake(dm9000lock,0); 			//请求互斥信号量,锁定DM9000 
...
void DMA9000_ISRHandler(void)
{
	...
	{  
 		xSemaphoreGiveFromISR(dm9000input, &xHigherPriorityTaskWoken);//接收完成中断，用户自行添加所需代码
	} 
